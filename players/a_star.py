from game import collision, lose, getPossibleMoves
...
class Agent:
    ...
    @staticmethod
    def aStar(snake: tuple[list[int]], apple: tuple[int, int], border: tuple[list[int]], display_range: int) -> list[tuple[int, int]]:
        ...
        try:
            if not len(Agent.current_path):
                Agent.current_path = Agent.aStar(
                    snake, apple, border, display_range)
            try:
                for i, neighbor in enumerate(neighbors):
